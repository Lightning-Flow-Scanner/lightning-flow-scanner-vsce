...
export class ScanFlowsCommand extends BaseCommand {
  ...
  public async execute() {
    ...
    if (selectedUris.length > 0) {
      ...
      let namingConvention: vscode.WorkspaceConfiguration = vscode.workspace.getConfiguration('lightningFlowScanner').get("namingConvention");
      let apiVersionEvalExpression: vscode.WorkspaceConfiguration = vscode.workspace.getConfiguration('lightningFlowScanner').get("versionAPI");
      if((namingConvention as unknown as string) == "0" || (apiVersionEvalExpression as unknown as string) == "0"){
        ...
        await vscode.workspace.getConfiguration().update('lightningFlowScanner.versionAPI', apiVersionEvalExpression, true);
      }
