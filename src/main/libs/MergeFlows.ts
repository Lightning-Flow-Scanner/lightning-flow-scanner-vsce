...
export class MergeFlows {
    ...
    private preProcessFlows(flows: Flow[]) {
        const mainNodes = this.preProcessFlowNodes(flows[0]);
        const secondaryNodes = this.preProcessFlowNodes(flows[1]);
        return [...mainNodes, ...secondaryNodes];
    }

    private preProcessFlowNodes(flow) {
        ...
        const flowXML = flow.xmldata.Flow;
        const allNodes = [];
        for (let nodeType in flowXML) {
            ...
            if (flowMetadata.includes(nodeType)) {
                ...
                for (let node of nodes) {
                    allNodes.push(
                        new FlowVariable(node.name, nodeType, flow.flownumber, node)
                    );
                }
                ...
                for (let node of nodes) {
                    allNodes.push(
                        new FlowElement(node.name, nodeType, flow.flownumber, node)
                    );
                }
            }
        }
        ...
        do {
            ...
            if (elementsToProcess.length > 0){
                for (const element of elementsToProcess){
                    const newElement: FlowElement = new FlowElement(element.name, element.subtype, element.flownumber, element.element);
    ...
    private async compareConnectors(element, matchingElement) {
        let allConnectors =
            matchingElement && matchingElement.connectors.length > 0
